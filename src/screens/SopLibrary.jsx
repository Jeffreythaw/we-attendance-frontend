...
export default function SopLibrary({ onAuthError }) {
  ...
  async function previewDocument(sopDocument, language = "en") {
    ...
    try {
      const data = await apiFetch(`/api/SopDocuments/${encodeURIComponent(sopDocument.id)}/preview?language=${encodeURIComponent(language)}`);
      setPreview({ id: sopDocument.id, title: data?.title || sopDocument.title, content: data?.content || "No readable text was found in this document.", language: data?.language || language, availableLanguages: sopDocument.availableLanguages || ["en"] });
    } catch (e) {
        ...
        {documents.map((document) => (
          <article className="we-sop-card" key={document.id}>
            ...
            <div className="we-sop-translation">
              ...
              <label className="we-sop-upload">{uploadingId === document.id ? "Uploading…" : "Upload translation"}<input type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" disabled={uploadingId === document.id} onChange={(event) => { uploadTranslation(document, event.target.files?.[0]); event.target.value = ""; }} /></label>
            </div>
          </article>
        ))}
      </div>
...
.we-sop-modal{position:fixed;inset:0;z-index:1000;display:grid;place-items:center;padding:20px;background:rgba(4,12,26,.6);}.we-sop-preview{width:min(880px,100%);max-height:min(82vh,900px);display:grid;grid-template-rows:auto 1fr;overflow:hidden;border:1px solid var(--we-border);border-radius:16px;background:var(--we-surface);box-shadow:0 24px 70px rgba(0,0,0,.34);}.we-sop-previewHead{display:flex;justify-content:space-between;gap:16px;padding:18px 20px;border-bottom:1px solid var(--we-border);}.we-sop-previewHead span{font-size:10px;font-weight:900;letter-spacing:.08em;color:var(--we-muted);}.we-sop-previewHead h2{margin:5px 0 0;font-size:20px;color:var(--we-text);}.we-sop-previewActions{display:flex;align-items:center;gap:10px;}.we-sop-previewActions label{display:grid;gap:3px;font-size:10px;font-weight:900;letter-spacing:.06em;color:var(--we-muted);}.we-sop-previewActions select{min-width:112px;border:1px solid var(--we-control-border);border-radius:8px;background:var(--we-control-bg);color:var(--we-text);padding:6px 8px;font:inherit;letter-spacing:normal;}.we-sop-previewActions select:disabled{opacity:.55;}.we-sop-previewHead button{width:32px;height:32px;border:0;border-radius:50%;background:var(--we-surface-2);color:var(--we-text);font-size:24px;line-height:1;cursor:pointer;}.we-sop-preview pre{overflow:auto;margin:0;padding:22px 24px;white-space:pre-wrap;font:14px/1.65 "SF Pro Text",-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--we-text);}
.we-sop-modal{background:rgba(2,8,22,.7);backdrop-filter:blur(4px);}.we-sop-preview{background:#14243b!important;border-color:#355271!important;color:#f5f8ff!important;}.we-sop-previewHead{background:#19304e;border-color:#355271!important;}.we-sop-previewHead span,.we-sop-previewActions label{color:#b8c9df!important;}.we-sop-previewHead h2,.we-sop-preview pre{color:#f5f8ff!important;}.we-sop-preview pre{background:#14243b;}.we-sop-previewActions select{background:#0f1d31!important;border-color:#496988!important;color:#f5f8ff!important;}.we-sop-previewHead button{background:#294766!important;color:#fff!important;}
html.theme-light .we-sop-preview{background:#fff!important;border-color:#cbd8e7!important;color:#142949!important;}.theme-light .we-sop-previewHead{background:#f5f8fc;border-color:#d9e3ef!important;}.theme-light .we-sop-previewHead span,.theme-light .we-sop-previewActions label{color:#60738d!important;}.theme-light .we-sop-previewHead h2,.theme-light .we-sop-preview pre{color:#142949!important;}.theme-light .we-sop-preview pre{background:#fff;}.theme-light .we-sop-previewActions select{background:#fff!important;border-color:#b9c9dc!important;color:#142949!important;}.theme-light .we-sop-previewHead button{background:#e7eef7!important;color:#142949!important;}
...
@media (max-width:620px){.we-sop-hero{align-items:flex-start;padding:18px;}.we-sop-grid{grid-template-columns:1fr;}.we-sop-hero h1{font-size:23px;}.we-sop-count{min-width:72px;}.we-sop-card{min-height:170px;}.we-sop-previewHead{align-items:flex-start;flex-direction:column;}.we-sop-previewActions{width:100%;justify-content:space-between;}}
`;
