...
export default function SopLibrary({ onAuthError }) {
  ...
  useEffect(() => {
    ...
    apiFetch("/api/SopDocuments")
      .then((rows) => {
        ...
        setDocuments(list);
        setTranslationsAvailable(!!list[0]?.translationsAvailable);
      })
      .catch((e) => {
        if (!active) return;
  ...
  async function previewDocument(sopDocument, language = "en") {
    ...
    try {
      ...
      setPreview({ id: sopDocument.id, title: data?.title || sopDocument.title, content: data?.content || "No readable text was found in this document.", language: data?.language || language });
    } catch (e) {
    ...
    <section className="we-sop-root">
      ...
      {preview ? <div className="we-sop-modal" role="dialog" aria-modal="true" aria-label={`${preview.title} preview`}>
        <div className="we-sop-preview">
          <div className="we-sop-previewHead"><div><span>SOP DOCUMENT</span><h2>{preview.title}</h2></div><div className="we-sop-previewActions"><label>Language<select value={preview.language} onChange={(event) => previewDocument({ id: preview.id, title: preview.title }, event.target.value)} disabled={!translationsAvailable || previewBusy === preview.id}><option value="en">English</option><option value="my">မြန်မာ</option><option value="zh-Hans">中文</option><option value="hi">हिन्दी</option><option value="bn">বাংলা</option></select></label><button type="button" onClick={() => setPreview(null)} aria-label="Close preview">×</button></div></div>
